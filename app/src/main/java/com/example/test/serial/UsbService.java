package com.example.test.serial;
...
public class UsbService extends Service {
    ...
    private static final String ACTION_USB_PERMISSION = "com.example.test.serial.USB_PERMISSION";
...
//                if (deviceVID != 0x1d6b && (devicePID != 0x0001 && devicePID != 0x0002 && devicePID != 0x0003) && deviceVID != 0x5c6 && devicePID != 0x904c) {
    ...
    private class ConnectionThread extends Thread {
        @Override
        public void run() {
                while(device != null&& device.getInterfaceCount() <= 0){
                    try {
                        sleep(2000);
                    ...
                    new Handler(getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(context,"itf count = "+device.getInterfaceCount(),Toast.LENGTH_SHORT).show();
                        }
                    });
                }
